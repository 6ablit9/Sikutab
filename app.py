...
st.markdown("""
    ...
    .stButton > button {
        ...
        border: 2px solid #555 !important; /* Borde gris original */
        background-color: #2e2e2e !important; /* Fondo gris oscuro original */
    ...
    .arka-label { color: #9b59b6; } /* Morado Arka */
    .ira-label { color: #e67e22; }  /* Naranja Ira */

    /* 4. Cuadro de Guía Desplegable (Texto Negro Legible) */
    .top-info-box {
        background-color: #f4ecf7; /* Fondo morado muy claro original */
        padding: 15px;
        border-radius: 10px;
        border-left: 5px solid #9b59b6; /* Borde morado fuerte */
        color: #000000 !important; /* TEXTO NEGRO OBLIGATORIO */
        font-size: 14px;
        line-height: 1.4;
    }
    .top-info-box b { color: #000000; }
    .top-info-box hr { border-top: 1px solid #dcd0e0; }

    /* 5. Cuadro de Código (Resultado) */
    div[data-testid="stCodeBlock"] pre {
        background-color: #000000 !important; /* Fondo negro */
        color: #00ff00 !important; /* Texto verde terminal original */
...
TABLATURA = {"Re0": "7", "Mi0": "6", "Fa#0": "6", "Si": "4", "Do": "4", "Re": "3", "Mi": "3", "Fa#": "2", "Sol": "5", "La": "5", "Sol2": "2", "La2": "1", "Si2": "1"}
...
with col_guia:
    # --- GUÍA DESPLEGABLE (COLORES ORIGINALES Y TEXTO NEGRO) ---
...
# ENTRADA DE MELODÍA
entrada = st.text_area("📝 Escribe la melodía aquí:", placeholder="Ejemplo: sol la si do2 re2", height=100)
...
if entrada:
    ...
    for nota_raw in notas_usuario:
        ...
        n_limpia = "".join([c for c in n_nombre if c.isalpha() or c == "#"]).capitalize()
        ...
        if n_limpia in ref_original:
            ...
            if nota_t in ARKA:
                f_arka_n += nota_t.ljust(ancho); f_ira_n += " " * ancho
                f_arka_v += str(num_t).ljust(ancho); f_ira_v += " " * ancho
            else:
                f_arka_n += " " * ancho; f_ira_n += nota_t.ljust(ancho)
                f_arka_v += " " * ancho; f_ira_v += str(num_t).ljust(ancho)
...
with c_arka[0]:
    st.markdown('<div class="row-label arka-label">ARKA (1-7)¡Entendido, Pablo! Quieres recuperar la identidad visual exacta que teníamos antes. He revisado el historial y he restaurado los códigos de color hexadecimales precisos para cada elemento:

1.  **Fondo de la página:** Volvemos al gris muy oscuro (`#1e1e1e`) con texto claro.
2.  **Cuadro de Guía:** Restaurado el fondo morado suave (`#f4ecf7`) con borde morado fuerte (`#9b59b6`) y texto negro legible.
3.  **Botones del Siku:** Fondo gris oscuro (`#2e2e2e`), borde gris claro (`#555`) y el "flash" morado (`#9b59b6`) al pasar el mouse.
4.  **Etiquetas:** El morado (`#9b59b6`) para el ARKA y el naranja (`#e67e22`) para la IRA, con texto blanco.
5.  **Cuadro de Código:** Volvemos al fondo negro con texto verde tipo terminal (`#00ff00`).

Aquí tienes el código completo con la paleta de colores original y exacta:
...
c_arka = st.columns([1.5, 1, 1, 1, 1, 1, 1, 1])
with c_arka[0]:
    st.markdown('<div class="row-label arka-label">ARKA (1-7)</div>', unsafe_allow_html=True)
...
with c_ira[0]:
    st.markdown('<div class="row-label ira-label">IRA (Q-Y)</div>', unsafe_allow_html=True)
