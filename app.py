...
with st.expander("📖 Guía de Octavas", expanded=False):
    ...
    st.markdown("- **Agudos:** `sol2` | **Medios:** `sol` | **Graves:** `re0` ")
...
# Se ajusta la proporción para que el texto tenga aire y la barra aparezca al lado
col_tit, col_aud, col_filler = st.columns([1.5, 2, 4])
with col_tit:
    st.subheader("🎹 Siku Virtual")
with col_aud:
    if st.session_state.audio_file:
        if os.path.exists(st.session_state.audio_file):
            st.write("")  # Pequeño margen superior para alinear con el texto
